...
impl ModInfo {
    pub(super) fn new(pak_path: PathBuf) -> Result<Self> {
        ...
        let entry = pak.entry("modinfo.json").ok_or(anyhow!("Missing modinfo.json"))?;
        if entry.encrypted {
        ...
        let json_bytes = if bom == 0xfeff {
            ...
            let utf8_len = ucs2::decode(&ucs2_buf, &mut utf8_bytes)
        ...
        Ok(Self { pak_path, ..serde_json::from_slice(&json_bytes)?})
    }
}
