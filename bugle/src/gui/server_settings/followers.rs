...
impl FollowersTab {
    pub fn new(settings: FollowerSettings) -> Rc<Self> {
        ...
        let pen_crafting_time_mult_prop = grid.range_prop(
            ...
            1.0,
            1000,
            settings.pen_crafting_time_mult,
        );
        let feeder_rang_mult_prop = grid.range_prop(
            "Food container range multiplier:",
            0.1,
            4.0,
            1.0,
            10,
            settings.feeder_rang_mult,
        );
        let cap_enabled_prop =
            grid.bool_prop("Use follower population limit", settings.cap_enabled);
        let cap_base_prop = grid.range_prop(
            "Follower population base value:",
            0.0,
            150.0,
            1.0,
            1,
            settings.cap_base as f64,
        );
        let cap_per_player_prop = grid.range_prop(
            "Follower population per player:",
            0.0,
            150.0,
            1.0,
            1,
            settings.cap_per_player as f64,
        );
        let cleanup_interval_prop = grid.range_prop(
            "Overpopulation cleanup interval:",
            5.0,
            720.0,
            1.0,
            1,
            settings.cleanup_interval.num_minutes() as f64,
        );
